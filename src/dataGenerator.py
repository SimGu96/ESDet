...
import numpy as np
import random
from src.utils_squeezeDet import bbox_transform_inv, batch_iou, sparse_to_dense
...
def read_image_and_gt(img_files, gt_files, config):
    ...
    for img_name, gt_name in zip(img_files, gt_files):
        ...
        img = tf.image.decode_image(open(img_name, 'rb').read(), channels=3)
        ...
        #and store
        imgs[img_idx] = img #np.asarray(img)
        ...
        #tf.print(np.isnan(np.sum(delta_per_image)), output_stream='file://loggtest3.txt')
        #tf.print(np.isnan(np.sum(aidx_per_image)), output_stream='file://loggtest4.txt')
        #tf.print(np.isnan(np.sum(labels_per_file)), output_stream='file://loggtest5.txt')
    ...
    #tf.debugging.assert_all_finite(Y, "NaN was found")
    return imgs, Y
